...
- **Headers follow the screen type — do NOT port the web header-card verbatim** (it is a desktop pattern; "parity of information, not of layout"): **list/index** screens (Home, Videos, Sessions, Groups) use `ZPageHeader` (compact title + optional one-line subtitle, no card) + a **FAB** for the primary create action + a header icon-button/overflow for secondary/config actions; **detail** screens use an entity hero card (avatar/name/status) with contextual actions; **form/wizard** screens use a header card (title + summary) + stepper. The FAB holds the screen's **primary action for the user's role** — `create` for creators (the `assets:create`/`groups:create`-gated pattern), or the role's equivalent primary action when create doesn't apply (e.g. **Join** is the Groups-tab FAB for students, who never see the create FAB — the two are mutually exclusive per user, so there is no clash; use a distinct icon, e.g. QR-scan for Join vs. `+` for Create). The primary action is a FAB, never a header button; genuinely secondary actions (and, for users who can do both, the non-primary one) go in the header.
