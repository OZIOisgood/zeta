 ...
 *
 * The backend alphabet is [A-Za-z0-9] (fixed length 6 today) and codes are
 * looked up case-sensitively, so the case MUST be preserved — do not
 * upper/lower-case the value. The 4–10 character window is tolerance for
...
export function parseInviteCode(data: string): string {
  ...
  try {
    ...
    if (code) {
      ...
      return /^[A-Za-z0-9]{4,10}$/.test(trimmed) ? trimmed : '';
    }
