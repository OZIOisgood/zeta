...
The API deploy maps these constants to `ALLOWED_ORIGINS`, `LANDING_ORIGIN`, `FRONTEND_URL`, `WORKOS_REDIRECT_URI`, and `RESEND_FROM_EMAIL`. `LANDING_ORIGIN` is the public origin allowed to call the unauthenticated contact endpoint. The dashboard deploy maps the API URL to browser-visible `API_URL`.
...
| `ANALYTICS_VIEWER_MEMBERS` | Comma-separated IAM principals with read-only BigQuery business analytics access |
...
- Dev BigQuery business analytics dataset, Cloud SQL connection, scheduled query,
  aggregate tables, and viewer IAM
...
The development business analytics dataset is managed by
`infra/terraform/modules/business-analytics` and is exposed by the
`business_analytics_dataset_url` Terraform output. It contains aggregate BigQuery
tables for video uploads, review comments, live coachings, and successful refresh
runs. BigQuery Data Transfer refreshes the tables from Cloud SQL through a
read-only federated connection; Looker Studio should connect only to the
materialized BigQuery tables.

Add viewers to the dev GitHub Environment variable `ANALYTICS_VIEWER_MEMBERS`
using principals such as `user:email@example.com` or `group:ops@example.com`.
The module grants BigQuery data viewer access on the analytics dataset and
BigQuery job user access for viewer-credential Looker Studio reports. The
Looker Studio report itself is created and shared manually with the same users or
group.

The analytics Cloud SQL user password is generated by Terraform and stored only
in Terraform state and the BigQuery connection credential. Do not create or share
a separate analytics password.
