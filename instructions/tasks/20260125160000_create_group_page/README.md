...
As a user with the `groups--create` feature, I want to be able to create a group. When clicking the "create group" tile, a new page should open (available only with the feature) with one form input for the group name.
...
- Feature flags are managed through the `FeatureService` and `groups--create` flag should be available
...
3. The create-group route is added to `app.routes.ts` with `featureGuard` and `data: { feature: 'groups--create' }`
