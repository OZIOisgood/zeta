...
- **Config ownership and parity** — use the infra-configuration skill; verify new env vars have one appropriate source of truth and are reflected in `.env.example`, application reads/tests, dev/prod bindings, Terraform, provider settings, and docs wherever applicable. Flag public values stored as secrets and secret payloads stored as plain env.
