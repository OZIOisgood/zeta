# Colofon

> **Opmerking:** De juridisch bindende versie van dit document is de Duitse versie. Deze Nederlandse versie is een niet-bindende servicevertaling; bij twijfel of tegenstrijdigheid prevaleert de Duitse versie.

Wij stellen via ons [contactformulier](/contact) een tweede, snel bereikbare contactmogelijkheid in de zin van artikel 5, lid 1, nr. 2, DDG ter beschikking, waarmee wij verzoeken op korte termijn beantwoorden.

## Btw-identificatienummer

Een btw-identificatienummer in de zin van artikel 27a van de Duitse Wet op de omzetbelasting is niet aanwezig. {{GBR_BEZEICHNUNG}} is niet ingeschreven in het handelsregister.

## Contactpunt conform artikel 11 van Verordening (EU) 2022/2065 (Digital Services Act – DSA)

Centraal contactpunt voor autoriteiten en rechterlijke instanties: {{EMAIL_DSA}}
Communicatie is mogelijk in het Duits en het Engels.

Het contactpunt voor gebruikers conform artikel 12 DSA is geregeld in onze [Gebruiksvoorwaarden](/gebruiksvoorwaarden).

## Consumentengeschillenbeslechting

Wij zijn niet bereid en niet verplicht om deel te nemen aan geschillenbeslechtingsprocedures voor een consumentengeschillencommissie (artikel 36 van de Duitse wet inzake consumentengeschillenbeslechting – VSBG).

> De voorheen verplichte link naar het Europese platform voor onlinegeschillenbeslechting (ODR-platform) is niet langer van toepassing; het platform is op 20 juli 2025 stopgezet.

<!-- Redactionele opmerking: een "inhoudelijk verantwoordelijke conform artikel 18, lid 2, MStV" is alleen vereist bij journalistiek-redactionele inhoud. Er wordt van uitgegaan dat Strido dergelijke inhoud niet aanbiedt; voeg anders hier de naam + het adres van een verantwoordelijke persoon toe. -->
