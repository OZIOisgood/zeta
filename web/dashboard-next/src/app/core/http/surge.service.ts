...
export class SurgeService {
  ...
  }: SurgeUpload): Observable<SurgeEvent> {
  ...
  private runXHR(
    ...
    xhr.onload = (): void => {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        this.emitError(
          correlationId,
          `Server returned ${xhr.status} for "${file.name}"`,
          xhr.statusText,
          upload$,
        );
      }
            ...
            if (err.status === 308) {
              ...
              if (rangeHeader) {
                ...
                if (!lastByte) {
                  this.emitError(
                    correlationId,
                    `Invalid Range header for "${file.name}"`,
                    err,
                    upload$,
                  );
                  return;
  ...
  private emitComplete(correlationId: string, upload$: BehaviorSubject<SurgeEvent>): void {
