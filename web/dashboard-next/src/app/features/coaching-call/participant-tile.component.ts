...
@Component({
  ...
  imports: [LucideMicOff, TranslocoPipe, ZAvatarComponent],
  ...
  template: `
    ...
    @if (!state().videoTrack) {
      <div class="absolute inset-0 grid place-items-center bg-stone-900 p-4 text-center text-white">
        <div class="grid justify-items-center gap-3">
          <z-avatar
            class="size-20 text-2xl shadow-lg sm:size-24"
    ...
    <div
      class="absolute inset-x-0 bottom-0 flex items-end justify-between gap-2 bg-gradient-to-t from-black/70 to-transparent p-3 pt-10 text-white"
    >
